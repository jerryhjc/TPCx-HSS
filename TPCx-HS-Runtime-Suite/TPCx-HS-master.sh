...
sudo -u $HDFS_USER /usr/lib/hadoop/bin/hadoop  fs -mkdir /user
sudo -u $HDFS_USER /usr/lib/hadoop/bin/hadoop  fs -mkdir /user/"$HADOOP_USER"
sudo -u $HDFS_USER /usr/lib/hadoop/bin/hadoop  fs -chown "$HADOOP_USER":"$HADOOP_USER" /user/"$HADOOP_USER"

/usr/lib/hadoop/bin/hadoop  fs -ls "${HDFS_BENCHMARK_DIR}"
if [ $? != 0 ] ;then
  /usr/lib/hadoop/bin/hadoop  fs -mkdir "${HDFS_BENCHMARK_DIR}"
fi
...
/usr/lib/hadoop/bin/hadoop  fs -rm -r -skipTrash /user/"$HADOOP_USER"/"${HDFS_BENCHMARK_DIR}"/*
sudo -u $HDFS_USER /usr/lib/hadoop/bin/hadoop  fs -expunge
...
if [ "$FRAMEWORK" = "MapReduce" ]; then
    (time /usr/lib/hadoop/bin/hadoop  jar $HSSORT_JAR HSGen -Dmapreduce.job.maps=$NUM_MAPS -Dmapreduce.job.reduces=$NUM_REDUCERS -Dmapred.map.tasks=$NUM_MAPS -Dmapred.reduce.tasks=$NUM_REDUCERS $hssize /user/"$HADOOP_USER"/"${HDFS_BENCHMARK_DIR}"/HSsort-input) 2> >(tee ./logs/HSgen-time-run$i.txt) 
else
...
if [ "$FRAMEWORK" = "MapReduce" ]; then
    (time /usr/lib/hadoop/bin/hadoop  jar $HSSORT_JAR HSValidate /user/"$HADOOP_USER"/"${HDFS_BENCHMARK_DIR}"/HSsort-output /user/"$HADOOP_USER"/"${HDFS_BENCHMARK_DIR}"/HSValidate) 2> >(tee ./logs/HSvalidate-time-run$i.txt) 
else
